            ...
            for (int i = 0; i < 100; i++) {
                ...
                if (i % 10 == 0) {
                    totals.add(executor.submit(() -> {
                        ...
                        return List.of(accounts.get().map(Account::getId).max(Long::compare).get(), accounts.getTotalElements());
                    }));
                }
            }
            ...
            for (int i = 0; i < 100; i++) {
                ...
                if (i % 10 == 0) {
                    totals.add(executor.submit(() -> {
                        ...
                        return List.of((long) accounts.get().count(), accounts.getTotalElements());
                    }));
                }
            }
